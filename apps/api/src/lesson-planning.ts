...
import { generateSchedule, type Lesson, type Slot } from "./schedule-engine";
...
export class LessonPlanningController {
  ...
  async settings(@Req() req: AuthRequest, @Body() body: unknown) {
    allowOperational(req.actor);
    allow(req.actor, "school.update");
    const input = z
      .object({ minutes: z.number().int().min(10).max(120) })
      .parse(body);
    await this.db.transaction(req.actor.tenant_id, (sql) =>
      sql.query(
        "UPDATE tenant_settings SET lesson_minutes=$2 WHERE tenant_id=$1",
        [req.actor.tenant_id, input.minutes],
      ),
    );
    return input;
  }
  ...
  async generate(@Req() req: AuthRequest, @Body() body: unknown) {
    allowOperational(req.actor);
    allow(req.actor, "academic.create");
    const input = optionsSchema.parse(body);
    if (input.apply) {
      allow(req.actor, "academic.update");
      allow(req.actor, "academic.delete");
    }
    const tenant = req.actor.tenant_id;

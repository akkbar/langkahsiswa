  ...
  @Post("copies") async copy(@Req() req: AuthRequest, @Body() body: unknown) {
    ...
    return (
      await this.db.query(
        ...
        [req.actor.tenant_id, x.book_id, x.barcode],
      )
    ).rows[0];
  }
    ...
    return this.db.transaction(req.actor.tenant_id, async (sql) => {
      ...
      const student = (
        await sql.query(
          ...
          [req.actor.tenant_id, x.student_id],
        )
      ).rows[0];
    ...
    return this.db.transaction(req.actor.tenant_id, async (sql) => {
      ...
      const result = (
        await sql.query(
          ...
          [req.actor.tenant_id, id, req.actor.id, status],
        )
      ).rows[0];
      await sql.query(
        "INSERT INTO library_returns(tenant_id,borrowing_id,condition,returned_by) VALUES($1,$2,$3,$4)",
        [req.actor.tenant_id, id, x.condition, req.actor.id],
      );
      await sql.query(
    ...
    return this.db.transaction(req.actor.tenant_id, async (sql) => {
      ...
      return (
        await sql.query(
          ...
          [req.actor.tenant_id, id, transaction.id, req.actor.id],
        )
      ).rows[0];
    });
  }

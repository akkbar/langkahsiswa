...
export class FinanceService {
  ...
  async students(actor: Actor, query: Record<string, unknown>) {
    financeReader(actor);
    const values = scopeValues(actor);
    ...
    return page(
      (
        await this.db.query(
          `SELECT s.id,s.name,s.nis,s.status,COALESCE(w.balance,0)::numeric AS balance,count(*) OVER() AS _total FROM students s LEFT JOIN wallet_accounts w ON w.tenant_id=s.tenant_id AND w.student_id=s.id WHERE s.tenant_id=$1 AND ${ownScope(actor)} AND (s.name ILIKE $${n - 2} OR s.nis ILIKE $${n - 2}) ORDER BY s.name,s.id LIMIT $${n - 1} OFFSET $${n}`,
          values,
        )
      ).rows,
      p.page,
  ...
  async addFee(actor: Actor, body: unknown) {
    manage(actor);
    const x = feeInput.parse(body);
    return (
      await this.db.query(
        "INSERT INTO fee_types(tenant_id,name,description,amount,active) VALUES($1,$2,$3,$4,$5) RETURNING *",
        [actor.tenant_id, x.name, x.description, x.amount, x.active],
      )
    ).rows[0];
  }
  async invoices(actor: Actor, query: Record<string, unknown>) {
    financeReader(actor);
    const p = paging(query),
    ...
    return page(
      (
        await this.db.query(
          `SELECT i.*,s.name AS student_name,s.nis,count(*) OVER() AS _total FROM invoices i JOIN students s ON s.tenant_id=i.tenant_id AND s.id=i.student_id WHERE i.tenant_id=$1 AND ${ownScope(actor)} ${filters} ORDER BY i.due_date,i.created_at DESC LIMIT $${values.length - 1} OFFSET $${values.length}`,
          values,
        )
      ).rows,
      p.page,
  ...
  async addInvoice(actor: Actor, body: unknown) {
    manage(actor);
    const x = z
      .object({
        student_id: uuid,
    ...
    return this.db.transaction(actor.tenant_id, async (sql) => {
      await studentAccess(sql, actor, x.student_id);
        ...
        if (
          ...
          !(
            await sql.query(
              ...
              [actor.tenant_id, item.fee_type_id],
            )
          ).rowCount
        )
      ...
      const invoice = (
        await sql.query(
          ...
          [actor.tenant_id, x.student_id, x.title, x.due_date, total, actor.id],
        )
      ).rows[0];
      invoice.items = [];
      for (const item of x.items)
  ...
  async invoice(actor: Actor, id: string) {
    ...
    await studentAccess(this.db, actor, invoice.student_id);
    invoice.items = (
      await this.db.query(
  ...
  async uploadProof(actor: Actor, body: unknown) {
    const x = proofInput.parse(body);
    await studentAccess(this.db, actor, x.student_id);
    if (
    ...
    const storageKey = `${actor.tenant_id}/${id}.${extension}`,
    ...
    try {
      ...
      return await this.db.transaction(actor.tenant_id, async (sql) => {
        ...
        await sql.query(
          ...
          [actor.tenant_id, id, x.student_id],
        );
        return proof;
      });
    } catch (error) {
  ...
  async downloadProof(actor: Actor, id: string, res: Response) {
    uuid.parse(id);
    const proof = (
      await this.db.query(
    ...
    await studentAccess(this.db, actor, proof.student_id);
    let bytes: Buffer;
    try {
  ...
  async submitPayment(actor: Actor, id: string, body: unknown) {
    uuid.parse(id);
    ...
    return this.db.transaction(actor.tenant_id, async (sql) => {
      ...
      if (!invoice) throw new NotFoundException("Tagihan tidak ditemukan");
      await studentAccess(sql, actor, invoice.student_id);
      ...
      await useProof(sql, actor, x.proof_id, invoice.student_id);
      return (
        await sql.query(
  ...
  async payments(actor: Actor, query: Record<string, unknown>) {
    return this.paymentList(actor, query, "payments");
  }
  ...
  async verifyPayment(actor: Actor, id: string, body: unknown) {
    manage(actor);
    uuid.parse(id);
    ...
    return this.db.transaction(actor.tenant_id, async (sql) => {
      ...
      if (x.decision === "APPROVED") {
        const invoice = (
          await sql.query(
            "SELECT * FROM invoices WHERE tenant_id=$1 AND id=$2 FOR UPDATE",
            [actor.tenant_id, payment.invoice_id],
          )
        ).rows[0];
  ...
  async wallets(actor: Actor, query: Record<string, unknown>) {
    financeReader(actor);
    const p = paging(query),
      values = scopeValues(actor);
    let filter = "";
    ...
    return page(
      (
        await this.db.query(
          `SELECT w.id,s.id AS student_id,s.name AS student_name,s.nis,COALESCE(w.balance,0)::numeric AS balance,count(*) OVER() AS _total FROM students s LEFT JOIN wallet_accounts w ON w.tenant_id=s.tenant_id AND w.student_id=s.id WHERE s.tenant_id=$1 AND ${ownScope(actor)} ${filter} ORDER BY s.name,s.id LIMIT $${values.length - 1} OFFSET $${values.length}`,
          values,
        )
      ).rows,
      p.page,
  ...
  async wallet(actor: Actor, studentId: string) {
    return this.db.transaction(actor.tenant_id, async (sql) => {
      const student = await studentAccess(sql, actor, studentId);
      const wallet = (
        await sql.query(
          "SELECT id,balance,updated_at FROM wallet_accounts WHERE tenant_id=$1 AND student_id=$2",
          [actor.tenant_id, studentId],
        )
      ).rows[0] || { id: null, balance: 0 };
      const totals = await spending(sql, actor.tenant_id, studentId);
      const transactions = (
        await sql.query(
      ...
      return {
        ...
        transactions,
        limits: await limits(sql, actor.tenant_id, studentId),
      };
    });
  }
  async transactions(
    ...
    await studentAccess(this.db, actor, studentId);
    const p = paging(query);
    return page(
      (
        await this.db.query(
          ...
          [actor.tenant_id, studentId, p.limit, p.offset],
        )
      ).rows,
      p.page,
      p.limit,
  ...
  async updateLimits(actor: Actor, studentId: string, body: unknown) {
    const x = limitsInput.parse(body);
    return this.db.transaction(actor.tenant_id, async (sql) => {
      await studentAccess(sql, actor, studentId, true);
      await sql.query(
      ...
      for (const merchantId of new Set(x.blocked_merchant_ids))
        await sql.query(
          ...
          [actor.tenant_id, studentId, merchantId],
        );
      return limits(sql, actor.tenant_id, studentId);
    });
  }
  async topups(actor: Actor, query: Record<string, unknown>) {
    return this.paymentList(actor, query, "wallet_topups");
  }
  async addTopup(actor: Actor, body: unknown) {
    const x = paymentInput.extend({ student_id: uuid }).parse(body);
    return this.db.transaction(actor.tenant_id, async (sql) => {
      await studentAccess(sql, actor, x.student_id);
      await useProof(sql, actor, x.proof_id, x.student_id);
      return (
        await sql.query(
  ...
  async verifyTopup(actor: Actor, id: string, body: unknown) {
    manage(actor);
    uuid.parse(id);
    const x = verification.parse(body);
    return this.db.transaction(actor.tenant_id, async (sql) => {
  ...
  async adjustment(actor: Actor, studentId: string, body: unknown) {
    manage(actor);
    const x = z
      .object({
        amount: z
    ...
    return this.db.transaction(actor.tenant_id, async (sql) => {
      await studentAccess(sql, actor, studentId);
      const old = await repeated(sql, actor, x.idempotency_key, hash);
      if (old) return old;
      return ledger(sql, actor, {
        studentId,
        type: "ADJUSTMENT",
        amount: x.amount,
  ...
  async addMerchant(actor: Actor, body: unknown) {
    manage(actor);
    const x = merchantInput.parse(body);
    return (
      await this.db.query(
        "INSERT INTO wallet_merchants(tenant_id,name,category,active) VALUES($1,$2,$3,$4) RETURNING *",
        [actor.tenant_id, x.name, x.category, x.active],
      )
    ).rows[0];
  }
  async editMerchant(actor: Actor, id: string, body: unknown) {
    manage(actor);
    uuid.parse(id);
    const x = merchantInput.partial().parse(body);
    return this.updateCatalog(actor, "wallet_merchants", id, x);
  }
  async products(actor: Actor, query: Record<string, unknown>) {
    financeReader(actor);
    const p = paging(query),
    ...
    return page(
      (
        await this.db.query(
          ...
          [actor.tenant_id, merchantId || null, p.limit, p.offset],
        )
      ).rows,
      p.page,
      p.limit,
  ...
  async addProduct(actor: Actor, body: unknown) {
    manage(actor);
  ...
  async editProduct(actor: Actor, id: string, body: unknown) {
    manage(actor);
    uuid.parse(id);
    ...
    return this.updateCatalog(actor, "products", id, x);
  }
  ...
  async posStudents(actor: Actor, query: Record<string, unknown>) {
    manage(actor);
    return this.students(actor, query);
  }
  async checkout(actor: Actor, body: unknown) {
    manage(actor);
    const x = z
      .object({
        student_id: uuid,
    ...
    return this.db.transaction(actor.tenant_id, async (sql) => {
      const student = await studentAccess(sql, actor, x.student_id);
      const old = await repeated(sql, actor, x.idempotency_key, hash);
      if (old)
        return {
          ...old,
      ...
      for (const item of x.items) {
        const product = (
          await sql.query(
            ...
            [actor.tenant_id, item.product_id, x.merchant_id],
          )
        ).rows[0];
        if (!product)
          throw new BadRequestException(
      ...
      money.parse(total);
      const policy = await limits(sql, actor.tenant_id, x.student_id);
      ...
      const transaction = await ledger(sql, actor, {
      ...
      for (const item of selected) {
        if (item.stock !== null)
          await sql.query(
            ...
            [actor.tenant_id, item.id, item.quantity],
          );
  ...
  async refund(actor: Actor, id: string, body: unknown) {
    manage(actor);
    uuid.parse(id);
    const x = z
      .object({

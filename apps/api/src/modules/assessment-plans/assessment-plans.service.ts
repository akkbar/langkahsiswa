...
export class AssessmentPlansService {
 ...
 WHERE cs.tenant_id=$1 AND ($3::boolean OR t.user_id=$2) ORDER BY sem.start_date DESC,c.name,s.name`,
  ...
  async get(actor: Actor, id: string) {
    ...
    const data = await plan(this.db, actor.tenant_id, id);
    ...
    const locked = Boolean(
      (
        await this.db.query(
          ...
          [actor.tenant_id, subject.class_id, subject.semester_id],
        )
      ).rowCount,
    );
  ...
  async save(actor: Actor, id: string, body: unknown) {
    ...
    return this.db.transaction(actor.tenant_id, async (sql) => {
      const tenant = actor.tenant_id;
      const subject = await teachSubject(sql, actor, uuid.parse(id));
      ...
      for (const item of input.items) {
        ...
        if (!item.id) allow(actor, "grade.create");
        else allow(actor, "grade.update");
      }
      if (removed.length) allow(actor, "grade.delete");

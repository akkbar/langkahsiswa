...
export class PortalService {
  ...
  private privileged(actor: Actor) {
    return isAdmin(actor) || actor.roles.includes("PRINCIPAL");
  }
  async students(actor: Actor) {
    const data = (
      await this.db.query(
  ...
  async overview(actor: Actor, studentId: string) {
    ...
    const broad = this.privileged(actor) || student.is_owner;
    const params = [actor.tenant_id, studentId, actor.id, broad];
  ...
  async teaching(actor: Actor) {
    if (!actor.roles.includes("TEACHER") && !this.privileged(actor))
    ...
    const data = (
     ...
     FROM class_subjects cs JOIN teachers t ON t.tenant_id=cs.tenant_id AND t.id=cs.teacher_id JOIN classes c ON c.tenant_id=cs.tenant_id AND c.id=cs.class_id JOIN subjects s ON s.tenant_id=cs.tenant_id AND s.id=cs.subject_id JOIN semesters sem ON sem.tenant_id=cs.tenant_id AND sem.id=cs.semester_id WHERE cs.tenant_id=$1 AND ($3 OR t.user_id=$2) ORDER BY c.name,s.name`,
        [actor.tenant_id, actor.id, this.privileged(actor)],
      )
    ).rows;
    return { data, total: data.length };
  }

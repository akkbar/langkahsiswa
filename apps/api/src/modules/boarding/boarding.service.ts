...
import { allow, isAdmin } from "../auth/permissions";
...
async function mayAccessStudent(sql: Sql, actor: Actor, studentId: string) {
  if (isAdmin(actor) || actor.permissions.includes("boarding.write")) return;
  const ok = (
    await sql.query(
      ...
      [actor.tenant_id, studentId, actor.id],
    )
  ).rowCount;
...
export class BoardingService {
  ...
  async module(actor: Actor, key: string, body: unknown) {
    allow(actor, "boarding.write");
    const moduleKey = z
      .enum([
        "ACADEMIC",
        "ATTENDANCE",
    ...
    const x = z
      ...
      .strict()
      .parse(body);
    await exists(this.db, "schools", actor.tenant_id, x.school_id);
    return (
      await this.db.query(
        `INSERT INTO school_modules(tenant_id,school_id,module_key,enabled,config)
         ...
         RETURNING *`,
        [actor.tenant_id, x.school_id, moduleKey, x.enabled, x.config],
      )
    ).rows[0];
  }
  async dormitory(actor: Actor, body: unknown) {
    allow(actor, "boarding.write");
    const x = z
      .object({
        name: text,
    ...
    return (
      await this.db.query(
        ...
        [actor.tenant_id, x.name, x.gender, x.description],
      )
    ).rows[0];
  }
  ...
  async bed(actor: Actor, body: unknown) {
    allow(actor, "boarding.write");
    ...
    return this.db.transaction(actor.tenant_id, async (sql) => {
      ...
      return (
        await sql.query(
          ...
          [actor.tenant_id, x.room_id, x.code],
        )
      ).rows[0];
    });
  }
  async assign(actor: Actor, body: unknown) {
    allow(actor, "boarding.write");
    const x = z
    ...
    return this.db.transaction(actor.tenant_id, async (sql) => {
      ...
      const assignment = (
        await sql.query(
          ...
          [actor.tenant_id, x.student_id, x.bed_id, x.start_date, actor.id],
        )
      ).rows[0];
      await sql.query(
  ...
  async endAssignment(actor: Actor, id: string, body: unknown) {
    allow(actor, "boarding.write");
    uuid.parse(id);
    ...
    return this.db.transaction(actor.tenant_id, async (sql) => {
      ...
      await sql.query(
        ...
        [actor.tenant_id, a.bed_id],
      );
      return a;
    });
  }
  async leave(actor: Actor, body: unknown) {
    const x = z
      .object({
        student_id: uuid,
        start_at: timestamp,
    ...
    return this.db.transaction(actor.tenant_id, async (sql) => {
      await mayAccessStudent(sql, actor, x.student_id);
      return (
        await sql.query(
          ...
          [
            ...
            x.pickup_name,
            createHash("sha256")
              .update(`${actor.tenant_id}:${x.student_id}:${x.start_at}`)
              .digest("hex")
              .slice(0, 24),
            actor.id,
          ],
  ...
  async reviewLeave(actor: Actor, id: string, body: unknown) {
    allow(actor, "boarding.write");
    uuid.parse(id);
    const x = z
    ...
    const row = (
      await this.db.query(
        ...
        [actor.tenant_id, id, x.decision, actor.id, x.notes],
      )
    ).rows[0];
    if (!row) throw new ConflictException("Izin tidak lagi menunggu review");
    return row;
  }
  async leaveStatus(actor: Actor, id: string, body: unknown) {
    allow(actor, "boarding.write");
    uuid.parse(id);
    const status = z
  ...
  async leaveGate(actor: Actor, body: unknown) {
    allow(actor, "boarding.write");
    const x = z
      .object({
  ...
  async visit(actor: Actor, body: unknown) {
    allow(actor, "boarding.write");
    const x = z
      .object({
        student_id: uuid,
  ...
  async visitStatus(actor: Actor, id: string, body: unknown) {
    allow(actor, "boarding.write");
    uuid.parse(id);
    const status = z
  ...
  async discipline(actor: Actor, body: unknown) {
    allow(actor, "boarding.write");
    const x = z
      .object({
        student_id: uuid,
  ...
  async tahfidz(actor: Actor, body: unknown) {
    allow(actor, "boarding.write");
    const x = z
      .object({
        student_id: uuid,
  ...
  async activity(actor: Actor, body: unknown) {
    allow(actor, "boarding.write");
    const x = z
      .object({
  ...
  async tahfidzTarget(actor: Actor, body: unknown) {
    allow(actor, "boarding.write");
    const x = z
      .object({
        student_id: uuid,
  ...
  async worshipHabit(actor: Actor, body: unknown) {
    allow(actor, "boarding.write");
    const x = z
    ...
    return (
      await this.db.query(
        ...
        [actor.tenant_id, x.school_id, x.name, x.category],
      )
    ).rows[0];
  }
  async worshipRecord(actor: Actor, body: unknown) {
    allow(actor, "boarding.write");
    const x = z
      .object({
        student_id: uuid,
  ...
  async character(actor: Actor, body: unknown) {
    allow(actor, "boarding.write");
    const x = z
      .object({
        student_id: uuid,
  ...
  async reviewCharacter(actor: Actor, id: string, body: unknown) {
    allow(actor, "boarding.write");
    uuid.parse(id);
    const decision = z.enum(["APPROVED", "REJECTED"]).parse(
      (
        body as {
          decision?: unknown;
        }
  ...
  async diniyahSubject(actor: Actor, body: unknown) {
    allow(actor, "boarding.write");
    const x = z
      .object({
        school_id: uuid,
    ...
    return (
      await this.db.query(
        ...
        [actor.tenant_id, x.school_id, x.name, x.book_name, x.teacher_id],
      )
    ).rows[0];
  }
  async diniyahProgress(actor: Actor, body: unknown) {
    allow(actor, "boarding.write");
    const x = z
      .object({
        student_id: uuid,
  ...
  async inspection(actor: Actor, body: unknown) {
    allow(actor, "boarding.write");
    const x = z
      .object({
        room_id: uuid,
  ...
  async laundry(actor: Actor, body: unknown) {
    allow(actor, "boarding.write");
    const x = z
      .object({
        student_id: uuid,
  ...
  async laundryStatus(actor: Actor, id: string, body: unknown) {
    allow(actor, "boarding.write");
    uuid.parse(id);
    const x = z
      .object({
  ...
  async chargeLaundry(actor: Actor, id: string) {
    allow(actor, "boarding.write");
    uuid.parse(id);
    return this.db.transaction(actor.tenant_id, async (sql) => {
      const order = (
      ...
      await sql.query(
        ...
        [actor.tenant_id, order.student_id],
      );
      const account = (
        await sql.query(
      ...
      return (
        await sql.query(
          ...
          [actor.tenant_id, id, transaction.id],
        )
      ).rows[0];
    });
  }

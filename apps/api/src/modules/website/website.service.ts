...
export class PublicWebsiteService {
  ...
  async page(tenantSlug: string, slug: string) {
    const page = (
      await this.db.query(
  ...
  async domainPage(domain: string, slug: string) {
    const page = (
      await this.db.query(
        `SELECT p.id,p.title,p.slug,v.version_number,v.content,v.published_at,
         COALESCE(ws.site_name,s.name) AS site_name,COALESCE(ws.tagline,'') AS tagline,
  ...
  async asset(tenantSlug: string, id: string, res: Response) {
    uuid.parse(id);
    const row = (
      await this.db.query(
...
export class WebsiteService {
  ...
  async updateSettings(actor: Actor, body: unknown) {
    ...
    const row = (
      await this.db.query(
        ...
        [actor.tenant_id, x.school_id, x.site_name, x.tagline, x.primary_color],
      )
    ).rows[0];
  ...
  async assetFile(actor: Actor, id: string, res: Response) {
    allow(actor, "website.read");
    uuid.parse(id);
    const row = (
      await this.db.query(
  ...
  async createPage(actor: Actor, body: unknown) {
    allow(actor, "website.write");
    const x = z
      .object({
        school_id: uuid,
  ...
  async version(actor: Actor, id: string, body: unknown) {
    allow(actor, "website.write");
    uuid.parse(id);
    ...
    return this.db.transaction(actor.tenant_id, async (sql) => {
      ...
      await validateAssets(sql, actor.tenant_id, content);
      return (
        await sql.query(
          ...
          [actor.tenant_id, id, JSON.stringify(content), actor.id],
        )
      ).rows[0];
    });
  }
  async publish(actor: Actor, id: string, body: unknown) {
    allow(actor, "website.write");
    uuid.parse(id);
    ...
    return this.db.transaction(actor.tenant_id, async (sql) => {
      const version = (
        await sql.query(
          ...
          [actor.tenant_id, x.version_id, id],
        )
      ).rows[0];
      if (!version)
  ...
  async addAsset(actor: Actor, body: unknown) {
    allow(actor, "website.write");
    const x = fileInput
    ...
    try {
      return (
        await this.db.query(
          ...
          [actor.tenant_id, file.id, x.name, x.alt_text, actor.id],
        )
      ).rows[0];
    } catch (error) {
      await this.db.query(

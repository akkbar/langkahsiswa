...
export class AcademicYearSetupService {
  ...
  async list(actor: Actor, pageValue = "1", limitValue = "20", search = "") {
    allowOperational(actor);
    allow(actor, "academic_setup.read");
    const page = Math.max(1, Number(pageValue) || 1);
    const limit = Math.min(100, Math.max(1, Number(limitValue) || 20));
    const params = [actor.tenant_id, `%${search}%`, limit, (page - 1) * limit];
  ...
  create(actor: Actor, body: unknown) {
    allowOperational(actor);
    allow(actor, "academic_setup.create");
    const input = createSchema.parse(body);
    return this.db
      .transaction(actor.tenant_id, async (sql) => {
        const school = await sql.query(
          "SELECT id FROM schools WHERE tenant_id=$1 AND id=$2",
          [actor.tenant_id, input.school_id],
        );
        if (!school.rowCount)
        ...
        if (input.source_academic_year_id) {
          const source = await sql.query(
            ...
            [actor.tenant_id, input.source_academic_year_id, input.school_id],
          );
          if (!source.rowCount)
            throw new BadRequestException(
              ...
              (
                await sql.query(
                 ...
                 WHERE old.tenant_id=$1 AND old.academic_year_id=$2 AND st.step_key=$3`,
                  [actor.tenant_id, input.source_academic_year_id, key],
                )
              ).rows[0]?.payload || {};
          }
          await sql.query(
        ...
        return findSetup(sql, actor.tenant_id, setup.id);
      })
      .catch((error: any) => {
        if (error?.code === "23505")
  ...
  async detail(actor: Actor, id: string) {
    ...
    const setup = await findSetup(this.db, actor.tenant_id, id);
    const [
      steps,
      semesters,
      checks,
      ...
      this.db.query(
        ...
        [actor.tenant_id, setup.academic_year_id],
      ),
      activationChecks(this.db, actor.tenant_id, setup),
      this.db.query(
        ...
        [actor.tenant_id, setup.school_id],
      ),
      this.db.query(
  ...
  updateStep(actor: Actor, id: string, stepKey: string, body: unknown) {
    allowOperational(actor);
    allow(actor, "academic_setup.update");
    z.string().uuid().parse(id);
    if (
    ...
    return this.db.transaction(actor.tenant_id, async (sql) => {
      ...
      await sql.query(
        ...
        [actor.tenant_id, id, Math.min(12, step.step_order + 1)],
      );
      return step;
    });
  }
  activate(actor: Actor, id: string) {
    ...
    return this.db.transaction(actor.tenant_id, async (sql) => {
      const setup = await findSetup(sql, actor.tenant_id, id);
      const result = await activationChecks(sql, actor.tenant_id, setup);
      ...
      await sql.query(
        ...
        [actor.tenant_id, setup.school_id, setup.academic_year_id],
      );
      await sql.query(
      ...
      return { ...(await findSetup(sql, actor.tenant_id, id)), ...result };
    });
  }
  remove(actor: Actor, id: string) {
    allowOperational(actor);
    allow(actor, "academic_setup.delete");
    z.string().uuid().parse(id);
    return this.db
      .transaction(actor.tenant_id, async (sql) => {
        const setup = await findSetup(sql, actor.tenant_id, id);

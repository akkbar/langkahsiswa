...
export class AdmissionsService {
  ...
  async applications(actor: Actor, query: Record<string, unknown>) {
    ...
    const rows = (
      await this.db.query(
        ...
        [actor.tenant_id, x.status || "", x.search, x.limit, offset],
      )
    ).rows;
    return {
      data: rows,
  ...
  async enroll(actor: Actor, id: string, body: unknown) {
    admissionManager(actor);
    uuid.parse(id);
    const x = z
      .object({

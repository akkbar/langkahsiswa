...
export class SitesService {
  ...
  async uploadPhoto(actor: Actor, id: string, body: unknown) {
    allow(actor, "site.write");
    const targetId = uuid.parse(id);
  ...
  async deletePhoto(actor: Actor, id: string, fileId: string) {
    allow(actor, "site.write");
    const targetId = uuid.parse(id);
  ...
  async photoFile(actor: Actor, id: string, fileId: string, res: Response) {
    allow(actor, "site.read");
    const targetId = uuid.parse(id);
    const photoId = uuid.parse(fileId);
    await assertSiteAccess(
    ...
    return this.db.transaction(actor.tenant_id, async (sql) => {
      ...
      if (input.principal_teacher_id) {
        const teacher = (
          await sql.query(
            "SELECT name FROM teachers WHERE tenant_id=$1 AND id=$2",
            [actor.tenant_id, input.principal_teacher_id],
          )
        ).rows[0];
  ...
  async switch(req: AuthRequest, res: Response, id: string) {

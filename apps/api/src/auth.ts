...
export const rolePermissions: Record<Role, string[]> = {
  SUPER_ADMIN: ["*"],
  SCHOOL_ADMIN: ["*"],
  PRINCIPAL: [...read, "report.approve", "report.publish"],
  TEACHER: [
    ...read,
    "attendance.write",
    "grade.write",
  ...
  FINANCE: ["student.read", "people.read", "school.read", "payment.verify"],
  PARENT: ["report.own"],
  STUDENT: ["report.own"],
};
...
export class AuthService {
  ...
  async tokens(sql: Sql, userId: string, tenantId: string) {
    const refresh = randomBytes(48).toString("base64url");
    await sql.query(
      "INSERT INTO refresh_tokens(tenant_id,user_id,token_hash,expires_at) VALUES($1,$2,$3,now()+interval '7 days')",
      [tenantId, userId, digest(refresh)],
    );
    return {
      access_token: jwt.sign({ tenant_id: tenantId }, jwtSecret(), {
        subject: userId,
        expiresIn: "15m",
        issuer: "schoolapp",
        audience: "schoolapp",
...
export class AuthController {
  ...
  @Post("login") async login(
    ...
    const tokens = await this.auth.db.transaction(tenantId, (sql) =>
      this.auth.tokens(sql, user.id, tenantId),
    );
  ...
  @Post("refresh") async refresh(
    ...
    const result = await this.auth.db.transaction(null, async (sql) => {
      ...
      return {
        ...(await this.auth.tokens(sql, old.user_id, old.tenant_id)),
        user: actor,
      };
    });
  ...
  @Post("logout") async logout(
    ...
    if (req.cookies?.schoolapp_refresh)
      await this.auth.db.query(
        "UPDATE refresh_tokens SET revoked_at=now() WHERE token_hash=$1",
        [digest(req.cookies.schoolapp_refresh)],
      );

...
function allowManageRoles(req: AuthRequest) {
  allowOperational(req.actor);
  allow(req.actor, "user.update");
  if (!req.actor.permissions.includes("*"))
  ...
  async create(@Req() req: AuthRequest, @Body() body: unknown) {
    ...
    return this.db.transaction(req.actor.tenant_id, async (sql) => {
      ...
      const duplicate = await sql.query(
        ...
        [input.name, req.actor.organization_id],
      );
  ...
  async update(
    ...
    return this.db.transaction(req.actor.tenant_id, async (sql) => {
      ...
      const duplicate = await sql.query(
        ...
        [input.name, id, req.actor.organization_id],
      );
      if (duplicate.rowCount)
        throw new BadRequestException("Nama role sudah digunakan");
      const permissions = await validatePermissions(
        ...
        for (const permission of permissions)
          await sql.query(
            `INSERT INTO organization_role_permissions(organization_id,role_id,permission_id)
             VALUES($1,$2,$3)`,
            [req.actor.organization_id, id, permission],
          );
      }
